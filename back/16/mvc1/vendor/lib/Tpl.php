...
class Tpl
{
     ...
     * @param bool $isInclude    模板文件是仅仅需要编译，还是先编译在包含进来
     * @param null $uri 为了让缓存文件名不重复，将文件名和uri拼接起来在md5一下，生成缓存文件名
     */
    function display($viewName, $isInclude = true, $uri = null)
    {
        ...
        if (!file_exists($cachePath)) {
            ...
            //如果缓存文件不存在
            //  如果缓存文件不存在编译模板文件，生成缓存文件
            //  如果缓存文件存在，第一个，判断缓存文件是否过期，第二个判断模板文件是否被修改过，如果模板文件被修改过，缓存文件需要重新生成
